...
interface Props {
  editItem: BlogResponse
  actionDone: boolean
  setOpenEditModule: (value: boolean) => void
...
const EditForm: React.FC<Props> = ({
  ...
  const formik = useFormik({
    initialValues: {
      title: `${editItem.attributes.title}`,
      description: `${editItem.attributes.description}`,
      content: `${editItem.attributes.content}`,
      author: 'admin',
    },
    onSubmit: async (values) => {
      try {
        const response = await blog.editBlog({ id: editItem.id, data: values })
